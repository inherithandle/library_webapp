...
(function($) {
	...
	/*	
	  var frame = this.find('iframe, frame')[0];
      var sheight = frame.contentWindow.document.documentElement.scrollHeight;
	  var cheight = frame.contentWindow.document.documentElement.clientHeight ;
   ...
   {	
	...
	$.fn.closeDialog_help = function() {
		/***  도움말페이지 확인버튼 후 처리 함수 ***/
		alert("주요한 유의사항은 한번더 상세히 확인하시기 바랍니다.")
		this.clearResult().closeDialog();
	}
	$.fn.joinDialog = function() {
		/***  회원가입 확인버튼 후 처리 함수 ***/
		this.clearResult().closeDialog();
	}
